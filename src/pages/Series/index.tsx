...
const Movies = () => {
  ...
  const moviesData = contextData.filter(item => item.category === "TV Series")
  ...
  useEffect(() => {
    if (search) {
      ...
      setDataExported(moviesData.filter(item => item.category === "TV Series")); // Restore original data when search is empty
    }
  }, [search]);
